...
"$SED_PROG" 'w/dev/stderr' a >out 2>err|| fail=1
...
: W
EOF
"$SED_PROG" -f lbl-prog lbl-in-exp > lbl-out || fail=1
compare_ lbl-in-exp lbl-out

#
# character classes (compile.c:snarf_char_class)
#
if [ "$STRICT_ERROR_CHECK" ]; then
	...
	# EOF
	returns_ 1 "$SED_PROG" '/[' </dev/null 2>err-op-bracket1 || fail=1
	...
	# EOF
	returns_ 1 "$SED_PROG" -f op-bracket-prog  </dev/null 2>err-op-bracket2 || fail=1
	...
	# EOF
	returns_ 1 "$SED_PROG" 's/[[.//' </dev/null 2>err-chr-class || fail=1
	...
	# EOF
	returns_ 1 "$SED_PROG" 's/[[:]]//' </dev/null 2>err-chr-class2 || fail=1
	...
	# sed: -e expression #1, char 2: unterminated address regex
	# EOF
	returns_ 1 "$SED_PROG" '/\' </dev/null 2>err-backslash-eof || fail=1
...
"$SED_PROG" -n '{{p}}' in-exp > out-braces-1 || fail=1
compare_ in-exp out-braces-1 || fail=1

"$SED_PROG" -n '{p}#foo' in-exp > out-braces-2 || fail=1
compare_ in-exp out-braces-2 || fail=1

# 'l' followed by closing braces, and '#'
printf 'X$\n' > exp-l || framework_failure_
"$SED_PROG" -n  '{l}' in-exp > out-l-braces || fail=1
compare_ exp-l out-l-braces || fail=1
"$SED_PROG" -n  'l#foo' in-exp > out-l-hash || fail=1
...
# (pending_text)
"$SED_PROG" -e 'a\' in-exp > out-unterm-a1 || fail=1
