...
env LC_ALL=en_US.UTF-8 "$SED_PROG" -f prog1 < /dev/null 2>err1 && fail=1
...
env LC_ALL=en_US.UTF-8 "$SED_PROG" -f prog2 </dev/null 2>err2 && fail=1
...
LC_ALL=C "$SED_PROG" -f prog2 <in2 >out2 || fail=1
...
LC_ALL=en_US.UTF-8 "$SED_PROG" -f prog3 <in3 >out3 || fail=1
compare_ exp3 out3 || fail=1

# Expect identical result in C locale
LC_ALL=C "$SED_PROG" -f prog3 <in3 >out4 || fail=1
