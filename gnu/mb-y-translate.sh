...
LC_ALL=en_US.UTF-8 "$SED_PROG" -f p1 <in1 >out1 || fail=1
...
EOF
	env LC_ALL=C "$SED_PROG" -f p1 </dev/null 2>err1 && fail=1
...
LC_ALL=en_US.UTF-8 "$SED_PROG" -f p2 <in2 >out2 || fail=1
...
EOF
	env LC_ALL=C "$SED_PROG" -f p2 </dev/null 2>err2 && fail=1
...
do
    ...
    do
        LC_ALL=$l "$SED_PROG" -f p$t <in$t >out$t-$l || fail=1
...
EOF
	...
	env LC_ALL=en_US.UTF-8 "$SED_PROG" -f p7 </dev/null 2>err7 && fail=1
...
EOF
	...
	env LC_ALL=en_US.UTF-8 "$SED_PROG" -f p8 </dev/null 2>err8 && fail=1
