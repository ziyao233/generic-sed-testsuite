...
"$SED_PROG" -f prog-d in-d > out-d || fail=1
...
"$SED_PROG" -f prog-o in-o > out-o || fail=1
...
"$SED_PROG" -f prog-x in-x > out-x || fail=1
...
"$SED_PROG" -f cnv-num-prog cnv-num-in > cnv-num-out || fail=1
